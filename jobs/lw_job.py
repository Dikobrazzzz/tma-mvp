...
def main():
    ...
    for day in sorted(df["Date"].unique()):
        ...
        # 3) Добор до TOTAL_WINNERS_PER_DAY
        already = len(picked_emails)
        need_more = max(0, TOTAL_WINNERS_PER_DAY - already)
        if need_more > 0:
            pool = df_day[df_day["group"].isin(WINNERS_COUNT.keys())].copy()
            ...
            if not pool.empty:
                ...
                print(
                    f"[{day}] top-up: need_more={need_more}, "
                    f"added={extra['email_norm'].nunique() if not extra.empty else 0}"
                )
            else:
