...
# -*- coding: utf-8 -*-
"""
Lucky Winners job — реализация механики из LW.ipynb (v2)
Изменения (эта версия):
- VIP gate для Group 4 опционален (ENV).
- Заглушка VIP: LW_FORCE_VIP_LEVEL -> жёстко проставляет VIP всем строкам (например, 3).
- Недобор по группам добирается из общего пула → ровно TOTAL_WINNERS_PER_DAY победителей.
- Устойчивое округление в копейках → ровно PRIZE в сумме за день.
"""
import os
import argparse
...
G1_STD_SHIFT = float(os.getenv("LW_G1_STD_SHIFT", "0.5"))
LOW_SIGMA = float(os.getenv("LW_LOW_SIGMA", "2.0"))
...
return mid.reset_index(drop=True), mu, sigma
...
return "Group 3"
if g < mu - LOW_SIGMA * sigma:
# VIP-гейтинг для Group 4 — опционально
if (not REQ_VIP_G4) or (vip >= VIP_G4_MIN):
return "Group 4"
return "Other"
...
"""Мягкий анти-стрик + небольшой шум. Без повторов в рамках дня."""
...
"""Округление сумм так, чтобы их итог был ровно 'total' (устойчиво к отрицат. остатку)."""
    amounts = np.array(amounts, dtype=float)
if len(amounts) == 0:
return amounts
    cents = np.rint(amounts * 100.0, dtype=np.float64) # стартовая «почти целевая»
    floors = np.floor(cents)
    remainder = int(round(total * 100 - floors.sum()))
if remainder > 0:
        frac = cents - floors
        order = np.argsort(-frac) # отдаём копейки у кого больше дробная часть
        floors[order[:remainder]] += 1
elif remainder < 0:
        frac = cents - floors
        order = np.argsort(frac) # забираем копейки у кого дробная часть минимальна
        floors[order[:(-remainder)]] -= 1
...
for gname, w in GROUP_WEIGHTS.items():
    ...
    raw = (PRIZE * out["adj_reward"] / total_adj) if total_adj > 0 else out["base_reward"].to_numpy()
# Ровно PRIZE с учётом копеек
...
if args.date_from and args.date_to:
    ...
    draw_id = args.draw_id or (d_to.date().isoformat())
...
# 1) Выбор по квотам в группах в группах
        day_wins_parts = []
        picked_emails = set()
...
# 2) Добор до TOTAL_WINNERS_PER_DAY из общего пула (без "Other")
...
if not pool.empty:
                extra = pick_group_winners(pool.assign(group="Any"), need_more, prev_winners, consec, "Any")
if not extra.empty:
                    picked_emails |= set(extra["email_norm"].unique())
                    day_wins_parts.append(extra)
print(f"[{day}] top-up: need_more={need_more}, added={extra['email_norm'].nunique() if not extra.empty else 0}")
else:
print(f"[{day}] top-up: pool is empty, cannot reach target={TOTAL_WINNERS_PER_DAY}")
if not day_wins_parts:
continue
        day_wins = pd.concat(day_wins_parts, ignore_index=True)
# 3) Награды с точной суммой PRIZE на день
...
# --- Добавлено: Триггер модалки Claim Bonus для победителей (insert/update claim_denied_oneoff) ---
...
# --- Удаляем все строки за этот draw_id, которых нет в текущем результате ---
    to_keep = result["email_norm"].dropna().unique().tolist()
with psycopg.connect(dsn()) as con, con.cursor() as cur:
# psycopg3 корректно передаёт Python-список как массив для =ANY()
        cur.execute("""
...
# === Экспорт в Google Sheets (строго за день FROM из БД lw_winners) ===
    GSHEET_ID = os.getenv("LW_GSHEET_ID")
    GSHEET_TAB = os.getenv("LW_GSHEET_TAB", "winners")
    GSHEET_CREDS = os.getenv("LW_GSHEET_CREDS", "/opt/tma-mvp/creds/sa_gsheets.json")
if GSHEET_ID and os.path.isfile(GSHEET_CREDS):
try:
from gsheets_export import upload_dataframe, winners_df_for_day
            day_for_export = d_from.date().isoformat() # строго день FROM
            export_df = winners_df_for_day(day_for_export)
            upload_dataframe(GSHEET_ID, GSHEET_TAB, export_df, GSHEET_CREDS)
print(f"[gsheets] uploaded {len(export_df)} rows to '{GSHEET_TAB}' from DB day='{day_for_export}'")
except Exception as e:
print(f"[gsheets][warn] export failed: {e}")
else:
print("[gsheets] skipped (LW_GSHEET_ID or creds not set)")
